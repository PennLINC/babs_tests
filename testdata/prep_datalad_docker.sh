...
# build:
docker build -t chenyingzhao/datalad:datalad${datalad_version} -f Dockerfile_datalad \
    --build-arg datalad_version=${datalad_version} . 

# test:
docker run --rm -ti chenyingzhao/datalad:datalad${datalad_version}
...
# push:
docker push chenyingzhao/datalad:datalad${datalad_version}